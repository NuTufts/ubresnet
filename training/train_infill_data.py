...
from ub_uresnet_infill_data import UResNetInfillData # copy of old ssnet
...
def main():
    ...
    if GPUMODE:
        model = UResNetInfillData(inplanes=32,input_channels=1,num_classes=1,showsizes=False )
        model.to(device=torch.device(DEVICE)) # put onto gpuid
    else:
        model = UResNetInfillData(inplanes=32,input_channels=1,num_classes=1)
