...
def main():
    ...
    if GPUMODE:
        ...
        model = UResNet(inplanes=32,input_channels=1,num_classes=NUM_CLASSES)
    ...
    print "The train batchsize is: ", batchsize_train, "    Will view ", itersize_train, "  Images per training iteration."
    print "The valid batchsize is: ", batchsize_valid, "    Will view ", itersize_valid, "  Images per validate iteration."
    ...
    NENTRIES_VALID = len(iovalid)
    print "Number of entries in training set: ",NENTRIES
    print "Number of entries in valid set: ",NENTRIES_VALID


    if NENTRIES>itersize_train:    #Changed from  >0:
        iter_per_epoch = NENTRIES/(itersize_train)
        if num_iters is None:
            # we set it by the number of request VALID
    ...
    if False:
        ...
        import cv2 as cv
        # for pix in range(00, 40):
        #     for pix2 in range(0,80):
        #         print label_t.numpy()[0,pix,pix2],
        #     print ""
        print "Device Count: ",torch.cuda.device_count()
        print "Device 0 Capability: ",torch.cuda.get_device_capability(DEVICE_IDS[0])
        #print "Device 1 Capability: ",torch.cuda.get_device_capability(DEVICE_IDS[1])
        ...
        for ii in range(start_iter, num_iters):
            ...
            try:
                train_ave_loss, train_ave_acc = train(iotrain, batchsize_train, model,
                                                      criterion, optimizer,
                                                      nbatches_per_itertrain, ii, NUM_CLASSES, trainbatches_per_print)
            except Exception,e:
            ...
            if ii%iter_per_valid==0:
                try:
                    prec1 = validate(iovalid, batchsize_valid, model, criterion, nbatches_per_itervalid, validbatches_per_print, ii, NUM_CLASSES)
                except Exception,e:
...
def train(train_loader, batchsize, model, criterion, optimizer, nbatches, iiter, nclasses, print_freq):
    ...
    for i in range(0,nbatches):
        ...
        if acc_values is not None:
            losses.update(loss.item()) # .data[0]
        ...
        if i % print_freq == 0:
            ...
            status2 =(acc_list[0].val,acc_list[0].avg,
                      acc_list[1].val,acc_list[1].avg,
                      acc_list[2].val,acc_list[2].avg,
                      acc_list[3].val,acc_list[3].avg)

            print "Accuracies:          Background %.3f (%.3f) Track %.3f (%.3f) Shower %.3f (%.3f) TrackEnd %.3f (%.3f)"%status2
    ...
    writer.add_scalars('data/train_accuracy', {'bg':     acc_list[0].avg,
                                               ...
                                               'tkend':  acc_list[3].avg,
                                               'total':  acc_list[-1].avg}, iiter )

    return losses.avg,acc_list[-1].avg


def validate(val_loader, batchsize, model, criterion, nbatches, print_freq, iiter, nclasses):
    """
    inputs
    ------
    ...
    for i in range(0,nbatches):
        ...
        acc_values = accuracy(pred_t,label_t)
        if acc_values is not None:
            losses.update(loss_t.item()) #.data[0]
            for iacc,acc in enumerate(acc_list):
                acc.update( acc_values[iacc] )
        else:
            nnone += 1
    ...
    return float(np.mean([acc_list[1].avg, acc_list[2].avg, acc_list[3].avg]))
...
def accuracy(output, target):
    ...
    for c in range(output.size(1)):
        if num_per_class[c]>0:
            ...
            # print corr_per_class[c], "     ", float(num_per_class[c])
            # print corr_per_class[c]/float(num_per_class[c])*100.0
            res.append( corr_per_class[c]/float(num_per_class[c])*100.0 )
        else:
            # print "Error HEREEEEEEEEEE"
            # print num_per_class[c]
            res.append( 0.0 )

    # totals
    res.append( 100.0*float(total_corr)/float(total_pix) )

    #print "Result Accuracies:   ", res[0], "    ",res[1], "    ",res[2], "    ",res[3], "    ",res[4]
...
def prep_data( larcvloader, train_or_valid, batchsize, width, height, src_adc_threshold ):
    ...
    outputs
    -------
    adc_t (Pytorch Tensor):   adc ADC
    labels_t  (Pytorch Variable): labelss image
    ...
    adc_t = torch.from_numpy( data["adc_%s"%(train_or_valid)].reshape( (batchsize,1,width,height) ) ) # adc image ADC
    labels_t  = torch.from_numpy( data["labels_%s"%(train_or_valid)].reshape(  (batchsize,width,height) ).astype(np.int) )   # target image ADC
    ...
    # apply threshold to adc ADC values. returns a byte mask
    #adc_t[ adc_t<src_adc_threshold ] = 0.0
    #labels_t[  adc_t<src_adc_threshold ] = 0

    adc_t=adc_t.to(device=torch.device(DEVICE))
    labels_t=labels_t.to( device=torch.device(DEVICE))
    weight_t=weight_t.to(device=torch.device(DEVICE))

    return adc_t,labels_t,weight_t
