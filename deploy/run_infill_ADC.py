...
def pixelloop(ientry,
    ...
    for rows in range(512):
        for cols in range(832):
            ...
            if labelbasic_t[ib,0,rows,cols] == 1.0:
                # fill overlay_t
                overlay_t[ib,0,rows,cols] = outpix
                # calculate accuracies
            ...
            if diff < 2.0:
                thresh_t[ib,0,rows,cols] = 0
            elif diff < 5.0:
                thresh_t[ib,0,rows,cols] = 1
            elif diff < 10.0:
                thresh_t[ib,0,rows,cols] = 2
            elif diff < 20.0:
                thresh_t[ib,0,rows,cols] = 3
            else:
                thresh_t[ib,0,rows,cols] = 4
    ...
    return ientry, averageacc2, averageacc5, averageacc10, averageacc20, h, h2
...
if __name__=="__main__":
    ...
    if len(sys.argv)>1:
        ...
        checkpoint_data = "/mnt/disk1/nutufts/kmason/ubresnet/training/yplane_21500.tar"
        batch_size = 1
        gpuid = 1
        checkpoint_gpuid = 0
        verbose = True
    ...
    for ibatch in range(nbatches):
        ...
        thresh_t = torch.from_numpy( data["weights_valid"].reshape( (batch_size,1,width,height) ) )
        #save a copy of adc for creating overlay
        overlay_t = torch.from_numpy( data["ADCmasked_valid"].reshape( (batch_size,1,width,height) ) )
        ...
        for ib in range(batch_size):
            ...
            inentry, averageacc2, averageacc5, averageacc10, averageacc20, h, h2 = pixelloop(ientry,
                                                                                    averageacc2,
                                                                                    averageacc5,
                                                                                    averageacc10,
                                                                                    averageacc20,
