...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1):
        ...
        if stride > 1:
            self.shortcut = nn.Conv2d(inplanes, planes, kernel_size=1,
...
class ConvTransposeLayer(nn.Module):
    def __init__(self, deconv_inplanes, deconv_outplanes, res_outplanes):
        ...
        self.deconv = nn.ConvTranspose2d(deconv_inplanes, deconv_outplanes,
                                         kernel_size=4, stride=2, padding=1,
                                         bias=False)
        self.res = DoubleResNet(res_outplanes + deconv_outplanes, res_outplanes,
                                stride=1)
...
class ASPP(nn.Module):
    def __init__(self, inplanes, outplanes=16, nkernels=16, showsizes=False):
        ...
        # Block 5
        self.B5_gp = torch.nn.MaxPool2d(kernel_size=3, stride=stride, padding=1,
...
class ASPP_ResNet(nn.Module):
    ...
    def __init__(self, num_classes=3, in_channels=3, inplanes=16, showsizes=True):
        self.inplanes = inplanes
        ...
        self.enc_layer3 = self._make_encoding_layer(self.inplanes * 4,
                                                    self.inplanes * 8,
                                                    stride=2)
        self.enc_layer4 = self._make_encoding_layer(self.inplanes * 8,
                                                    self.inplanes * 16,
                                                    stride=2)
        self.enc_layer5 = self._make_encoding_layer(self.inplanes * 16,
                                                    self.inplanes * 32,
                                                    stride=2)
        ...
        ######################################################################
        # Decoding Layer (512->256, 256->128, 128->64, 64->32, 32->16)
        ######################################################################
        self.dec_layer5 = self._make_decoding_layer(self.inplanes * 64,
                                                    self.inplanes * 16,
                                                    self.inplanes * 32)
        self.dec_layer4 = self._make_decoding_layer(self.inplanes * 32,
                                                    self.inplanes * 8,
                                                    self.inplanes * 16)
        self.dec_layer3 = self._make_decoding_layer(self.inplanes * 16,
                                                    self.inplanes * 4,
                                                    self.inplanes * 4)
    ...
    def forward(self, x):
        ...
        ######################################################################
        # ASPP & Concatenated Skip Connections
        ######################################################################
        e3_ASPP = self.ASPP_layer_enc3(e3)
        if self.showsizes:
            print "e3_ASPP size after ASPP_layer:", e3_ASPP.size()

        e3_ASPP = self.ASPP_combine_enc3(e3_ASPP)
        if self.showsizes:
            print "e3_ASPP size after ASPP_combine:", e3_ASPP.size()

        e3_skip = torch.cat((e3_ASPP, e3), 1)
        if self.showsizes:
            print "e4_skip size after cat:", e3_skip.size()
